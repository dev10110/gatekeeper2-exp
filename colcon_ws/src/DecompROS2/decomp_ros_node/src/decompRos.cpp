...
// 		: Node("decompROS", node_options)
	...
	debug_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud>("fov_obs", 10);
		...
		point_cloud_sub_ = this->create_subscription<PointCloud2>("cloud_in", qos,
				std::bind(&DecompROS::cloud_callback, this, _1));
	...
	void DecompROS::cloud_callback(const PointCloud2::SharedPtr msg) const 
	{
		...
		// ok seems reasonable, lets try creating a polygon - need to publish geometry_msgs::msgs::PolygonStampled
		geometry_msgs::msg::PolygonStamped msg_poly;
		msg_poly.header.stamp = msg->header.stamp; 
		msg_poly.header.frame_id = msg->header.frame_id; // use the original pointclouds frame id
